...
def test_landing_returns_all_faculty_destinations():
    ...
    assert all("programmes" not in faculty for faculty in faculties)
    assert all({"key", "name", "description", "available"} <= set(faculty) for faculty in faculties)


def test_faculty_destination_serves_frontend():
    response = client.get("/faculty/uct_humanities")
    assert response.status_code == 200
    assert "Select your faculty" in response.text
