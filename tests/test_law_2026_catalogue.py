...
class LawCatalogueTests(unittest.TestCase):
    ...
    def passed(scoped, codes, mark=76, year=2026):
        ...
        for code in codes:
            ...
            rows.append(CourseResult(code, fact.name, fact.nqf_level, fact.nqf_credits, mark, None, year))
        return rows
    ...
    def test_routes_and_scopes_are_verified(self):
        self.assertEqual(
            ...
            {
                "llb_three_year_graduate", "llb_two_year_combined",
                "llb_four_year_undergraduate", "llb_five_year_continuing",
            },
        )
        self.assertEqual(self.catalogue.programmes["llb_five_year_continuing"].availability, "continuing_only")
        for key, programme in self.catalogue.programmes.items():
            pathways = list(programme.pathways) or [""]
            for pathway in pathways:
                _, scope = build_programme_scope("uct_law", self.catalogue, key, pathway)
    ...
    def test_complete_three_year_graduate_route_is_eligible(self):
        ...
        codes = programme.required_courses + ["CML4401H", "CML4602S", "PBL4502F", "PVL4505F"]
        student = StudentRecord(
            "1", "Law Student", "LLB three-year graduate", [],
            self.passed(scoped, codes), "uct_law", "llb_three_year_graduate", "", 3,
        )
    ...
    def test_research_elective_is_required(self):
        ...
        student = StudentRecord(
            ...
            "uct_law", "llb_three_year_graduate", "", 3,
        )
        report = compute_report(student, scoped)
        research = next(r for r in report.requirements if r.id == "curriculum:research_requirement")
    ...
    def test_final_electives_cannot_exceed_54_credits(self):
        scoped = self.scoped("llb_three_year_graduate")
        programme = scoped.programmes["llb_three_year_graduate"]
        electives = ["CML4401H", "CML4602S", "PBL4502F", "PVL4505F", "CML4501S", "CML4504S", "CML4506F"]
        student = StudentRecord(
            "3", "Too Many Electives", "LLB", [],
            self.passed(scoped, programme.required_courses + electives),
            "uct_law", "llb_three_year_graduate", "", 3,
        )
        report = compute_report(student, scoped)
        maximum = next(r for r in report.requirements if r.id == "curriculum:maximum_final_electives")
    ...
    def test_four_year_numeracy_pathways_are_separate(self):
        with_mam = self.scoped("llb_four_year_undergraduate", "numeracy_course_required")
        passed_test = self.scoped("llb_four_year_undergraduate", "numeracy_test_passed")
        self.assertIn("MAM1013F", with_mam.programmes["llb_four_year_undergraduate"].pathways["numeracy_course_required"].required_courses)
        self.assertNotIn("MAM1013F", passed_test.programmes["llb_four_year_undergraduate"].pathways["numeracy_test_passed"].required_courses)

    def test_route_specific_preliminary_prerequisites(self):
        graduate = self.scoped("llb_three_year_graduate")
        undergraduate = self.scoped("llb_four_year_undergraduate", "numeracy_course_required")
    ...
    def test_four_year_route_can_be_completed_with_known_nonlaw_courses(self):
        ...
        nonlaw = [
            "ELL1013F", "MAM1013F", "AFS1100S", "ANS1400F",
            "SLL1060F", "SLL1061S", "ELL2000F", "ELL2001S",
        ]
        electives = ["CML4401H", "CML4602S", "PBL4502F", "PVL4505F"]
        student = StudentRecord(
            "4", "Undergraduate", "Bachelor of Laws", [],
            self.passed(scoped, programme.required_courses + nonlaw + electives),
            "uct_law", "llb_four_year_undergraduate", "numeracy_course_required", 4,
        )
        report = compute_report(student, scoped)
    ...
    def test_four_failed_half_course_equivalents_trigger_risk(self):
        ...
        for code in failed_codes:
            fact = scoped.courses[code]
            rows.append(CourseResult(code, fact.name, fact.nqf_level, fact.nqf_credits, 40, None, 2026))
        student = StudentRecord(
            "5", "At Risk", "LLB", [], rows,
            "uct_law", "llb_three_year_graduate", "", 3,
        )
    ...
    def test_law_repeat_warning_does_not_cite_humanities_rule(self):
        ...
        student = StudentRecord(
            "6", "Repeat", "LLB", [],
            [
                CourseResult(fact.code, fact.name, fact.nqf_level, fact.nqf_credits, 40, None, 2025),
                CourseResult(fact.code, fact.name, fact.nqf_level, fact.nqf_credits, 45, None, 2026),
            ], "uct_law", "llb_three_year_graduate", "", 2,
        )
        report = compute_report(student, scoped)
