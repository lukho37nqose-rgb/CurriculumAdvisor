...
class ReportCreditsRequirementTests(unittest.TestCase):
    def test_report_credits_requirement_includes_reasoning_trace(self):
        ...
        self.assertEqual(credits_req.current, 18)
        self.assertEqual(credits_req.status, "provisional")
        self.assertEqual(credits_req.confidence, 0.95)
        self.assertEqual(credits_req.applied_rules, ["REGULAR_PROGRAMME_TOTAL_NQF_CREDITS"])
        self.assertEqual(credits_req.depends_on, ["credit_awarded:PHI1024F"])
        self.assertEqual(credits_req.assumptions, ["Programme rules inferred from programme title."])
