...
class ProgrammeScope:
    ...
    course_roles: dict[str, tuple[str, ...]]
    status: str
    warnings: tuple[str, ...]
...
def build_programme_scope(
    ...
    programme_required_codes: set[str] = set(programme.required_courses)
    programme_required_codes.update(
        collect_curriculum_course_codes(programme.curriculum_rules, catalogue)
    )
    pathway_required_codes: set[str] = set()
    if selected_pathway:
        pathway_required_codes.update(selected_pathway.required_courses)
        pathway_required_codes.update(
            collect_curriculum_course_codes(
                selected_pathway.curriculum_rules,
                catalogue,
            )
        )

    major_requirement_codes: set[str] = set()
    for major in scoped_majors.values():
        major_requirement_codes.update(major.required_courses)
        for group in major.choice_groups:
            major_requirement_codes.update(group.courses)
        major_requirement_codes.update(
    ...
    required_course_codes = (
        ...
        | pathway_required_codes
        | major_requirement_codes
    )
    ...
    course_roles: dict[str, tuple[str, ...]] = {}
    for code in allowed_course_codes:
        roles: list[str] = []
        if code in programme_required_codes or code in pathway_required_codes:
            roles.append("required")
        if code in support_course_codes:
            roles.append("support")
        if code in major_requirement_codes:
            roles.append("major_requirement")
        if code in explicit_electives:
            roles.append("elective")
        course_roles[code] = tuple(roles or ["required"])
    ...
    scope = ProgrammeScope(
        ...
        course_roles={
            code: course_roles[code]
            for code in sorted(scoped_courses)
        },
        status=status,
