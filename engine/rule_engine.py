...
from .models import Catalogue, CourseFact, CourseResult, MajorDefinition, StudentRecord
...
from .utils import _course_weight, _infer_programme_key, _is_senior, _normalise_major_keys
...
def _compute_major_progress(
    ...
    if major_def.curriculum_rules and catalogue is not None:
        ...
        if major_def.admission_limited:
            status = "unverified"
            outstanding_reqs.append(major_def.admission_note or "Formal admission to this major must be confirmed.")
        return MajorProgress(
            key=major_def.key, name=major_def.name, complete=complete,
            completed_requirements=completed_reqs, outstanding_requirements=outstanding_reqs,
            status=status, confidence=1.0 if status == "verified" else 0.45,
            used_course_codes=sorted(set(used_codes)),
        )

    if not major_def.required_courses and not major_def.choice_groups:
        return MajorProgress(
            key=major_def.key, name=major_def.name, complete=False,
            completed_requirements=[],
            outstanding_requirements=[
                "Requirements are not representable in the current catalogue; manual verification is required."
            ],
            status="unverified", confidence=0.0, used_course_codes=[],
        )
    ...
    if status != "verified":
        outstanding_reqs.extend(major_def.verification_notes or [
...
def _curriculum_option_labels(rules: list[dict]) -> dict[str, str]:
    ...
    def walk(rule: dict, optional_context: bool = False) -> None:
        ...
        optional_here = optional_context or rule_type in {
            "choose_n", "credit_pool", "approved_credit_pool",
            "same_department_credit_pool", "maximum_credit_pool", "any_of",
        }
        if optional_here:
...
def _compute_eligible_courses(
    ...
    if programme:
        ...
        if pathway:
            ...
            programme_required_codes.update(collect_curriculum_course_codes(pathway.curriculum_rules, catalogue))
    ...
    for code, course in catalogue.courses.items():
        ...
        for m_key, m_def in major_defs:
            ...
            if code in curriculum_codes:
                ...
                reason = (
                    ...
                    if option_label else f"Required for {m_def.name} major"
                )
                status = "verified" if (
        ...
        eligible.append(EligibleCourse(
            ...
            limitations=[
                ...
                *(
                    ["Co-requisite course(s) must be taken with this course: " + ", ".join(course.co_requisites)]
                    if course.co_requisites else []
                ),
                *(
                    [course.recognition_note]
                    if course.recognition_note else []
                ),
                "Timetable clashes, class limits, concessions, and faculty approval are not verified.",
            ],
        ))

    eligible.sort(key=lambda c: (not c.is_major_requirement, c.status != "verified", not _is_senior(c.code), c.code))
    return eligible
...
def _compute_exclusion_risk(
    ...
    if not prog:
        return ExclusionRisk(
            at_risk=False,
            reasons=["No verified readmission threshold table is available because no programme rules were identified."],
            assessed=False, status="unverified", confidence=0.0,
    ...
    cumulative_credits = (
        sum(fact.nqf_credits for _, fact in recognised)
        + sum(result.nqf_credits for result in provisional_progression)
    )
    ...
    if active_progression_rules:
        ...
        annual_passed = passed_by_year.get(latest_year, 0) if latest_year is not None else None
        annual_attempted = attempted_by_year.get(latest_year, 0) if latest_year is not None else None
        science_credits = sum(fact.nqf_credits for _, fact in recognised if fact.counts_as_science)
        senior_credits = sum(fact.nqf_credits for _, fact in recognised if fact.nqf_level >= 6)
        course_equivalents = sum(_course_weight(result.code) for result, fact in recognised if fact.counts_towards_general_degree)
        senior_course_equivalents = sum(
            _course_weight(result.code) for result, fact in recognised
        ...
        for raw in active_progression_rules:
            ...
            if rule_type == "annual_credits":
                ...
                if annual_passed is None:
                    unknown.append(f"{label}: the transcript does not expose academic-year labels needed to isolate annual credits.")
                    ...
                    notes.append(f"{label}: annual recognised credits meet the {minimum}-credit threshold.")

            elif rule_type == "annual_course_equivalents":
                minimum = float(raw.get("minimum", 0))
                if latest_year is None:
                    unknown.append(f"{label}: academic-year labels are required to isolate annual course equivalents.")
                else:
                    latest_passed: set[str] = set()
                    equivalents = 0.0
                    for result in student.results:
                        if result.academic_year != latest_year or not result.is_passed() or result.code in latest_passed:
                            continue
                        fact = catalogue.courses.get(result.code)
                    ...
                    if equivalents < minimum:
                        reasons.append(f"{label}: {equivalents:g} semester-course equivalent(s) passed in {latest_year}; at least {minimum:g} are ordinarily required.")
                    else:
                        notes.append(f"{label}: {equivalents:g} annual course equivalent(s) meet {minimum:g}.")

            elif rule_type == "cumulative_failed_course_equivalents":
                threshold = float(raw.get("threshold", raw.get("minimum", 7)))
                failed_equivalents = sum(_course_weight(result.code) for result in student.results if result.is_failed())
                if failed_equivalents >= threshold:
                    reasons.append(f"{label}: {failed_equivalents:g} failed semester-course equivalent attempt(s) are recorded; the published threshold is {threshold:g}.")
                else:
                    notes.append(f"{label}: {failed_equivalents:g} failed equivalent attempt(s) is below {threshold:g}.")

            elif rule_type == "cumulative_credits":
                minimum = int(raw.get("minimum", 0))
                if cumulative_credits < minimum:
                    reasons.append(f"{label}: {cumulative_credits} cumulative recognised credits; at least {minimum} are required.")
                else:
                    notes.append(f"{label}: cumulative recognised credits meet the {minimum}-credit threshold.")

            elif rule_type == "science_cumulative_credits":
                minimum = int(raw.get("minimum", 0))
                if science_credits < minimum:
                    reasons.append(f"{label}: {science_credits} recognised Science credits; at least {minimum} are required.")
                else:
                    notes.append(f"{label}: Science credits meet the {minimum}-credit threshold.")

            elif rule_type == "senior_cumulative_credits":
                minimum = int(raw.get("minimum", 0))
                if senior_credits < minimum:
                    reasons.append(f"{label}: {senior_credits} senior credits; at least {minimum} are required.")
                else:
                    notes.append(f"{label}: senior credits meet the {minimum}-credit threshold.")

            elif rule_type == "course_equivalents_cumulative":
                minimum = float(raw.get("minimum", 0))
                if course_equivalents < minimum:
                    reasons.append(f"{label}: {course_equivalents:g} full-course equivalents; at least {minimum:g} are required.")
                else:
                ...
                if senior_course_equivalents < minimum:
                    reasons.append(f"{label}: {senior_course_equivalents:g} senior full-course equivalents; at least {minimum:g} are required.")
                else:
                    notes.append(f"{label}: senior full-course equivalents meet {minimum:g}.")

            elif rule_type == "selected_major_stage_complete":
                stage = str(raw.get("stage", "year1"))
                required_count = int(raw.get("required", len(_normalise_major_keys(student.declared_majors, catalogue)) or 1))
                ...
                if not selected:
                    ...
                    if completed_count < required_count:
                        reasons.append(f"{label}: {completed_count} of {required_count} selected-major stage requirement(s) completed.")
                    else:
                ...
                if years is None:
                    ...
                    selected = _normalise_major_keys(student.declared_majors, catalogue)
                    majors_done = [
                        _compute_major_progress(catalogue.majors[key], student, catalogue=catalogue).complete
                        for key in selected if key in catalogue.majors
                    ]
                ...
                if annual_attempted in (None, 0):
                    unknown.append(f"{label}: annual attempted credits are unavailable.")
                else:
                    rate = (annual_passed or 0) / annual_attempted
                    if rate < minimum:
                        reasons.append(f"{label}: {rate * 100:.1f}% of annual attempted credits passed; at least {minimum * 100:.0f}% is required.")
                    else:
                        notes.append(f"{label}: annual pass rate meets {minimum * 100:.0f}%.")

            elif rule_type == "failed_any":
                codes = {str(code).strip().upper() for code in raw.get("course_codes", [])}
                failed = sorted(code for code in codes if any(r.code == code and r.is_failed() for r in student.results))
                if failed:
                ...
                exceeded = sorted(code for code, count in counts.items() if count > maximum)
                if exceeded:
                    reasons.append(f"{label}: more than {maximum} failed attempt(s) recorded for {', '.join(exceeded)}.")

            elif rule_type == "failed_course_equivalents":
                threshold = float(raw.get("threshold", raw.get("minimum", 4)))
                if latest_year is None:
                    unknown.append(f"{label}: academic-year labels are required to isolate failures in the latest year.")
                else:
                    failed_codes: set[str] = set()
                    failed_equivalents = 0.0
                    for result in student.results:
                        if result.academic_year != latest_year or not result.is_failed():
                            continue
                    ...
                    if failed_equivalents >= threshold:
                        ...
                        notes.append(f"{label}: {failed_equivalents:g} failed equivalent(s) is below {threshold:g}.")

            elif rule_type == "failed_course_count":
                threshold = int(raw.get("threshold", raw.get("minimum", 2)))
                course_codes = {str(code).strip().upper() for code in raw.get("course_codes", []) if str(code).strip()}
                if latest_year is None:
                    unknown.append(f"{label}: academic-year labels are required to isolate failures in the latest year.")
                else:
                    failed = {
                        result.code for result in student.results
                        if result.academic_year == latest_year and result.is_failed()
                    ...
                    if len(failed) >= threshold:
                        ...
                        notes.append(f"{label}: {len(failed)} failed course(s) is below {threshold}.")

            elif rule_type == "failed_course_fraction":
                threshold = float(raw.get("threshold", raw.get("minimum", 0.5)))
                course_codes = {str(code).strip().upper() for code in raw.get("course_codes", []) if str(code).strip()}
                if latest_year is None:
                    unknown.append(f"{label}: academic-year labels are required to isolate the latest academic year.")
                else:
                    latest_results: dict[str, CourseResult] = {}
                    for result in student.results:
                        if result.academic_year == latest_year and not result.is_pending():
                            if not course_codes or result.code in course_codes:
                                latest_results[result.code] = result
                    if not latest_results:
                        unknown.append(f"{label}: no completed course results are labelled for {latest_year}.")
                    else:
                        failed = sum(1 for result in latest_results.values() if result.is_failed())
                        ...
                        if fraction >= threshold:
                            ...
                            notes.append(f"{label}: annual failed-course proportion is {fraction * 100:.1f}%.")
                ...
                if latest_year is None:
                    ...
                    triggered: list[str] = []
                    for group in groups:
                        group_codes = {str(code).strip().upper() for code in group.get("course_codes", []) if str(code).strip()}
                        threshold = int(group.get("threshold", 2))
                        failed = sorted({
                            result.code for result in student.results
                            if result.academic_year == latest_year and result.is_failed() and result.code in group_codes
                        })
                        if len(failed) >= threshold:
                            triggered.append(
                ...
                if not rules:
                    ...
                    evaluations = CurriculumEvaluator(student, catalogue).evaluate_many(rules)
                    ...
                    if incomplete:
                        reasons.append(f"{label}: outstanding: " + "; ".join(incomplete[:8]) + ("…" if len(incomplete) > 8 else "") + ".")
                    elif unresolved:
                        unknown.append(f"{label}: completion depends on unresolved rule(s): " + ", ".join(unresolved[:8]) + ".")
                    else:
                ...
                if latest_year is None:
                    ...
                    earlier_codes = {
                        result.code for result in student.results
                        if result.academic_year is not None and result.academic_year < latest_year and not result.is_pending()
                    }
                    repeated_in_latest = {
                        result.code for result in student.results
                        if result.academic_year == latest_year and result.code in earlier_codes and not result.is_pending()
                    }
                    ...
                    if repeated_in_latest and failed_latest:
                        ...
                        notes.append(f"{label}: no repeat-year failure pattern is evident from the labelled results.")
                ...
                if years is None:
                    ...
                    reasons.append(f"{label}: {years} years registered exceeds the ordinary {maximum}-year limit.")
                else:
        ...
        return ExclusionRisk(
            ...
            basis="; ".join(basis_parts) or "Insufficient temporal evidence for the published progression rules.",
        )
    ...
    credited = [
        (result, fact) for result, fact in recognised
    ...
    return ExclusionRisk(
        at_risk=bool(reasons), reasons=reasons, assessed=True,
        status=status, confidence=confidence, basis=basis,
    )
...
def _compute_distinction(
    ...
    if catalogue.faculty_key == "uct_science" or (programme is not None and programme.programme_type == "science_degree"):
        evaluator = CurriculumEvaluator(student, catalogue)
        subjects: list[SubjectDistinction] = []
        ...
        for key in major_keys:
            ...
            if not major.award_rules:
                subjects.append(SubjectDistinction(
                    major=major.name, average=0.0, senior_courses_assessed=0,
                    eligible=False, status="unverified",
            ...
            numeric = [result.mark for result in marked if result is not None and result.mark is not None]
            ...
            subjects.append(SubjectDistinction(
                major=major.name, average=round(average, 1),
                senior_courses_assessed=len(used), eligible=complete and status == "verified",
        ...
        for result, fact in recognised:
            attempts = [r for r in student.results if r.code == result.code and not r.is_pending()]
        ...
        first_rows = [(result, fact) for result, fact in eligible_rows if result.mark >= 75]
        first_credits = sum(fact.nqf_credits for result, fact in first_rows)
        senior_first_credits = sum(fact.nqf_credits for result, fact in first_rows if fact.nqf_level >= 6)
        ...
        for credit_value, count in ((18, 6), (24, 6), (36, 4)):
            group = sorted(
                [result.mark for result, fact in eligible_rows if fact.nqf_credits == credit_value and result.mark is not None],
                reverse=True,
            )[:count]
            alternate_groups.append(len(group) == count and sum(group) / count >= 75)
        duration_known = student.years_registered is not None
        duration_met = duration_known and programme is not None and student.years_registered <= programme.minimum_duration_years
        ...
        return Distinction(
            ...
            confidence=0.95 if qualification_eligible else (0.45 if provisional else 0.85),
            reason=reason,
        )
    ...
    if programme is not None and programme.programme_type != "general_degree":
        ...
        if not structured_award_rules:
            return Distinction(
                qualification_eligible=False, provisional=True, subjects=[],
        ...
        for award in structured_award_rules:
            ...
            award_rows.append(SubjectDistinction(
                major=name,
                average=display_average,
                senior_courses_assessed=len({code for check in checks for code in check.used_course_codes}),
    ...
    def first_attempt_pass(code: str) -> bool:
        attempts = [
            result for result in student.results
    ...
    for key in major_keys:
        ...
        level7_codes = [
            code for code in senior_codes
            if catalogue.courses[code].nqf_level == 7
        ]

        if major_def.verification_status != "verified":
            subjects.append(subject_record(
                major_def.name, senior_codes, False, "unverified",
        ...
        if key == "economics":
            ...
            considered = ["ECO3020F"] + [code for code in chosen if code != "ECO3020F"][:2]
            marks = [marked_result(code).mark for code in considered if marked_result(code)]
            eligible = (
                progress.complete and required and len(marks) == 3
                and sum(marks) / 3 >= 80
                and sum(mark >= 75 for mark in marks) >= 2
            )
            subjects.append(subject_record(
                major_def.name, considered, eligible, "verified",
        ...
        if key == "informatics":
            ...
            alternatives = [code for code in ("INF2010S", "INF3012S") if marked_result(code)]
            considered = fixed + alternatives[:1]
            marks = [marked_result(code).mark for code in considered if marked_result(code)]
            level7_marks = [marked_result(code).mark for code in ("INF3011F", "INF3014F") if marked_result(code)]
            eligible = (
                len(marks) == 4 and min(marks) >= 70 and sum(marks) / 4 >= 75
                and len(level7_marks) == 2 and sum(level7_marks) / 2 >= 75
            )
            subjects.append(subject_record(
                major_def.name, considered, eligible, "unverified",
                "FB1.2 is represented, but the current Informatics major pathway itself remains provisional."
            ))
            continue

        if key == "law":
            considered = [code for code in used if code.startswith(("PBL", "PVL", "CML"))]
            marked = [code for code in considered if marked_result(code)]
            credit_total = sum(catalogue.courses[code].nqf_credits for code in marked if code in catalogue.courses)
            average = (
                sum(marked_result(code).mark * catalogue.courses[code].nqf_credits for code in marked) / credit_total
                if credit_total else 0.0
            )
            eligible = progress.complete and len(marked) == len(considered) == 6 and average >= 75
            subjects.append(SubjectDistinction(
                major=major_def.name, average=round(average, 1),
                senior_courses_assessed=len(considered), eligible=eligible,
                status="verified", reason="FB1.2 requires a credit-weighted average of at least 75% across all six Law-major courses."
            ))
            continue
        ...
        if key == "industrial_and_organisational_psychology":
            considered = [
                code for code in used
                if (fact := catalogue.courses.get(code)) is not None and fact.nqf_level in {6, 7}
            ]
            level6 = [code for code in considered if catalogue.courses[code].nqf_level == 6]
            level7 = [code for code in considered if catalogue.courses[code].nqf_level == 7]
            marks = [marked_result(code).mark for code in considered if marked_result(code)]
            eligible = (
                progress.complete and len(level6) >= 2 and len(level7) >= 2
                and len(marks) == len(considered) and all(mark >= 75 for mark in marks)
            )
            subjects.append(subject_record(
                major_def.name, considered, eligible, "verified",
        ...
        if key == "psychology":
            ...
            third_options = [
                code for code in level7_codes if code != "PSY3007S"
            ]
            considered = ["PSY2015F"] + second_options[:1] + ["PSY3007S"] + third_options[:1]
            marks = [marked_result(code).mark for code in considered if marked_result(code)]
            eligible = progress.complete and len(marks) == 4 and all(mark >= 75 for mark in marks)
            subjects.append(subject_record(
                major_def.name, considered, eligible, "verified",
        ...
        marks = [marked_result(code).mark for code in senior_codes if marked_result(code)]
        first_attempt = all(first_attempt_pass(code) for code in senior_codes)
        eligible = (
            progress.complete and weights >= 4 and level7_weights >= 2
            and len(marks) == len(senior_codes)
            and bool(marks) and min(marks) >= 70
        ...
        subjects.append(subject_record(major_def.name, senior_codes, eligible, status, reason))

    recognised, _ = recognised_credited_pairs(student, catalogue)
    first_class_sce = sum(
        _course_weight(result.code) for result, fact in recognised
        if fact.counts_towards_course_equivalents and result.mark is not None and result.mark >= 75
    )
    first_class_senior_sce = sum(
        _course_weight(result.code) for result, fact in recognised
        if fact.counts_towards_course_equivalents and fact.nqf_level >= 6
        and result.mark is not None and result.mark >= 75
    )
    subject_distinction = any(subject.eligible for subject in subjects)
    qualification_eligible = (
        first_class_sce >= 10
        and first_class_senior_sce >= 8
        and subject_distinction
    )
...
def _compute_warnings(
    ...
    unknown_passes = sorted(
        result.code for result in student.credited_results()
    ...
    if catalogue.faculty_key == "uct_science":
        ...
        for key in major_keys:
            ...
            if major is not None and major.admission_limited:
                warnings.append(major.admission_note or f"Admission to {major.name} must be confirmed by the relevant department.")
    ...
    additional_codes = [
        result.code for result, fact in recognised
...
def _compute_credits(
    ...
    credits_completed = (
        sum(fact.nqf_credits for _, fact in recognised)
        + sum(result.nqf_credits for result in provisional)
    )
    level_7_credits = (
        sum(fact.nqf_credits for _, fact in recognised if fact.nqf_level == 7)
...
def _compute_course_equivalents(student: StudentRecord, catalogue: Catalogue) -> tuple[float, float, float]:
    ...
    humanities_sce = sum(
        _course_weight(result.code) for result, fact in credited if fact.counts_as_humanities
    )
...
def _compute_all_major_progresses(student: StudentRecord, catalogue: Catalogue, major_keys: list[str]) -> tuple[list[MajorProgress], int, int]:
    ...
    for key in major_keys:
        ...
        if major_def:
            ...
            major_progresses.append(_compute_major_progress(major_def, student, base_graph, catalogue))
    ...
    humanities_majors_complete = sum(
        ...
        catalogue.majors[m.key].faculty_owned
    )
...
def _build_requirements(
    ...
    requirements: list[Requirement] = [
        Requirement(
            id="duration", label=f"Minimum {min_years} years of study",
            complete=duration_complete, current=duration_current, required=float(min_years),
            detail=duration_detail, status=duration_status,
    ...
    if prog and prog.maximum_registration_years:
        ...
        requirements.append(Requirement(
            id="maximum_registration", label="Maximum registration period",
            complete=max_complete, current=max_current,
            required=float(prog.maximum_registration_years), detail=max_detail,
            status=max_status, confidence=1.0 if student.years_registered is not None else 0.0,
            blocking=False,
        ))
    if semester_course_equivalents:
        requirements.append(Requirement(
            id="courses", label="Semester course equivalents",
            complete=sce_total >= semester_course_equivalents,
            current=sce_total, required=float(semester_course_equivalents),
            detail=f"{sce_total:.1f} of {semester_course_equivalents} required semester-course equivalents passed",
            status=programme_status, confidence=programme_confidence,
        ))
    if senior_course_equivalents:
        requirements.append(Requirement(
            id="senior", label="Senior semester courses",
            complete=senior_sce >= senior_course_equivalents,
            current=senior_sce, required=float(senior_course_equivalents),
    ...
    if humanities_course_equivalents:
        requirements.append(Requirement(
            id="humanities", label="Humanities semester courses",
            complete=humanities_sce >= humanities_course_equivalents,
            current=humanities_sce, required=float(humanities_course_equivalents),
    ...
    total_credits_graph = build_total_nqf_credits_graph(
        student=student, required_credits=total_nqf_credits,
        programme_key=programme_key, programme_name=prog.name if prog else programme_key,
        assumptions=credit_assumptions, catalogue=catalogue,
    ...
    requirements.extend([
        Requirement(
            id="credits", label="NQF credits",
            complete=credits_completed >= total_nqf_credits,
            current=float(credits_completed), required=float(total_nqf_credits),
            detail=(
                ...
                + (
                    ...
                    if provisional_credit_results else ""
                )
            ),
            evidence=credit_conclusion.evidence, applied_rules=credit_conclusion.applied_rules,
    ...
    if level_7_nqf_credits:
        requirements.append(Requirement(
            id="level7", label="NQF Level 7 credits",
            complete=level_7_credits >= level_7_nqf_credits,
            current=float(level_7_credits), required=float(level_7_nqf_credits),
            detail=f"{level_7_credits} of {level_7_nqf_credits} NQF Level 7 credits completed",
            status=programme_status, confidence=programme_confidence,
        ))
    if required_majors:
        requirements.append(Requirement(
            id="majors", label="Completed majors",
            complete=majors_complete >= required_majors,
            current=float(majors_complete), required=float(required_majors),
            detail=f"{majors_complete} of {required_majors} majors completed",
            status="unverified" if any(m.status != "verified" for m in major_progresses) else programme_status,
            confidence=0.4 if any(m.status != "verified" for m in major_progresses) else programme_confidence,
        ))
    ...
    if catalogue.faculty_key == "uct_science":
        ...
        for key in sorted(selected_major_keys):
            ...
            if major is None or not major.required_co_majors:
                continue
            missing = [required for required in major.required_co_majors if required not in selected_major_keys]
            requirements.append(Requirement(
                id=f"major_co_requirement:{key}",
                label=f"Required co-major for {major.name}",
                complete=not missing, current=float(len(major.required_co_majors)-len(missing)),
                required=float(len(major.required_co_majors)),
                detail=(
                    f"{major.name} must be taken with " + ", ".join(catalogue.majors[m].name if m in catalogue.majors else m for m in major.required_co_majors) + "."
    ...
    if required_humanities_majors:
        requirements.append(Requirement(
            id="humanities_major", label="At least one Humanities major",
            complete=humanities_majors_complete >= required_humanities_majors,
            current=float(humanities_majors_complete), required=float(required_humanities_majors),
    ...
    faculty_categories = {
        catalogue.majors[key].qualification for key in declared_keys
    ...
    if prog and prog.required_majors > 0 and prog.degree_category in {"BA", "BSocSc"}:
        requirements.append(Requirement(
            id="degree_major_identity", label="Majors match the selected degree title",
            complete=degree_match, current=1.0 if degree_match else 0.0, required=1.0,
            detail=(
                "Mixed BA and BSocSc Humanities majors may lead to either degree title."
                if degree_match else
                "The selected degree title does not match the category of the declared Humanities major(s), or no Humanities major was identified."
            ), status=programme_status, confidence=programme_confidence,
        ))
    ...
    if prog:
        for code in prog.required_courses:
            requirements.append(Requirement(
                id=f"programme_course:{code}", label=f"Required programme course: {code}",
                complete=code in passed, current=1.0 if code in passed else 0.0, required=1.0,
                detail=f"{code} is compulsory for {prog.name}.",
                status=programme_status, confidence=programme_confidence,
            ))
        if prog.introductory_courses_required:
            intro_done = sum(1 for code in prog.introductory_course_options if code in passed)
            requirements.append(Requirement(
                id="extended_introductory", label="Extended-programme introductory courses",
                complete=intro_done >= prog.introductory_courses_required,
                current=float(intro_done), required=float(prog.introductory_courses_required),
        ...
        if prog.augmenting_courses_required:
            augmenting_done = sum(
                1 for code in passed
                if (fact := catalogue.courses.get(code)) is not None and fact.augmenting
            )
            requirements.append(Requirement(
                id="extended_augmenting", label="Extended-programme augmenting courses",
                complete=augmenting_done >= prog.augmenting_courses_required,
                current=float(augmenting_done), required=float(prog.augmenting_courses_required),
    ...
    if catalogue.faculty_key == "uct_science":
        overlap_credits = sum(
            catalogue.courses[code].nqf_credits for code in overlaps
        ...
        if not overlaps:
            ...
            overlap_complete, overlap_status = True, "discretionary"
            overlap_detail = (
                f"{overlap_credits} level-7 credits overlap. A three-major allocation may be approved where at least 180 level-7 credits are completed; Deputy Dean approval is required."
            )
        else:
        ...
        requirements.append(Requirement(
            id="major_course_allocation", label="Distinct level-7 credits across Science majors",
            complete=overlap_complete, current=float(overlap_credits), required=36.0,
            detail=overlap_detail, status=overlap_status,
            confidence=1.0 if overlap_status == "verified" else 0.0,
        ))
        requirements.append(Requirement(
            id="major_combination", label="Valid Science major combination",
            complete=forbidden_ok, current=1.0 if forbidden_ok else 0.0, required=1.0,
            detail="" if forbidden_ok else "Applied Statistics, Mathematical Statistics, and Statistics & Data Science may not be combined with one another.",
            status="verified", confidence=1.0,
        ))
    elif prog is None or prog.programme_type == "general_degree":
        requirements.append(Requirement(
            id="major_course_allocation", label="No senior course double-counted across majors",
            complete=True, current=0.0 if overlaps else 1.0, required=1.0,
            detail=(
                "Manual allocation is required for: " + "; ".join(f"{code} ({', '.join(names)})" for code, names in overlaps.items())
                if overlaps else "No senior-course overlap was detected in the courses allocated by the engine."
            ),
            status="unverified" if overlaps else "verified",
            confidence=0.4 if overlaps else 1.0,
        ))
        requirements.append(Requirement(
            id="major_combination", label="Valid major combination",
            complete=forbidden_ok, current=1.0 if forbidden_ok else 0.0, required=1.0,
            detail="" if forbidden_ok else "The selected majors are a forbidden combination under FB3(e).",
            status="verified", confidence=1.0,
        ))
    requirements.append(Requirement(
        id="qualification_match", label="Programme rules identified",
        complete=prog is not None, current=1.0 if prog else 0.0, required=1.0,
...
def compute_report(student: StudentRecord, catalogue: Catalogue) -> Report:
    ...
    credits_completed, level_7_credits, provisional_credit_results = _compute_credits(student, catalogue)
    ...
    requirements = _build_requirements(
        ...
        credits_completed, level_7_credits, provisional_credit_results,
        major_progresses,
