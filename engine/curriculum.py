...
def collect_rule_course_codes(rule: dict[str, Any], catalogue: Catalogue | None = None) -> set[str]:
...
def _fact_matches_filters(code: str, fact: CourseFact, filters: dict[str, Any]) -> bool:
    ...
    if explicit and code not in explicit:
        return False
    prefixes = [str(p).strip().upper() for p in filters.get("prefixes", []) if str(p).strip()]
    if prefixes and not any(code.startswith(prefix) for prefix in prefixes):
        return False
    excluded_prefixes = [
        str(p).strip().upper() for p in filters.get("exclude_prefixes", []) if str(p).strip()
    ]
    if excluded_prefixes and any(code.startswith(prefix) for prefix in excluded_prefixes):
        return False
...
class CurriculumEvaluator:
    def __init__(self, student: StudentRecord, catalogue: Catalogue):
        ...
        self.open_credit_allocations = provisional_open_credit_allocations(student, catalogue)
        self.open_allocation_by_id = {allocation.rule_id: allocation for allocation in self.open_credit_allocations}
        self.provisional_results = [
            result for allocation in self.open_credit_allocations for result in allocation.results
        ]
        self.provisional_result_by_code = {result.code: result for result in self.provisional_results}
        self.provisional_codes = set(self.provisional_result_by_code)
        self.credits = (
            sum(fact.nqf_credits for _, fact in pairs)
            + sum(result.nqf_credits for result in self.provisional_results)
        )
        self.credits_by_level: dict[int, int] = {}
        for _, fact in pairs:
            self.credits_by_level[fact.nqf_level] = self.credits_by_level.get(fact.nqf_level, 0) + fact.nqf_credits
        for result in self.provisional_results:
            self.credits_by_level[result.nqf_level] = self.credits_by_level.get(result.nqf_level, 0) + result.nqf_credits
    ...
    def evaluate(self, rule: dict[str, Any]) -> RuleEvaluation:
        ...
        if rule_type == "manual":
            ...
            note = str(rule.get("note", "This condition requires confirmation by the relevant academic authority."))
            return RuleEvaluation(
                id=rule_id, label=label, complete=assumed_complete,
                current=1.0 if assumed_complete else 0.0, required=1.0,
                detail=note, status=manual_status, confidence=0.0,
                blocking=blocking, assumptions=[note], source=source,
            )
        ...
        if rule_type == "credits":
            ...
            if self.provisional_codes:
                ...
                detail += " Provisional elective codes: " + ", ".join(sorted(self.provisional_codes)) + "."
            return RuleEvaluation(
                rule_id, label, current >= required, current, required, detail,
                evaluation_status, 0.45 if self.provisional_codes else (1.0 if status == "verified" else 0.7), blocking,
                used_course_codes=sorted(self.passed | self.provisional_codes), assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "level_credits":
            ...
            used = sorted(code for code, fact in self.fact_by_code.items() if fact.nqf_level == level)
            return RuleEvaluation(
                rule_id, label, current >= required, current, required,
                f"{int(current)} of {int(required)} credits at NQF level {level} completed.",
                status, 1.0 if status == "verified" else 0.7, blocking,
                used_course_codes=used, source=source,
            )
        ...
        if rule_type == "minimum_mark":
            ...
            matched = [self.result_by_code[code] for code in codes if code in self.result_by_code]
            if not matched:
                return RuleEvaluation(
                    rule_id, label, False, 0.0, float(threshold),
                    f"No passing result was found for any of: {', '.join(codes)}.",
                    status, 1.0, blocking, source=source,
                )
            ...
            if not known_marks:
                ...
                return RuleEvaluation(
                    rule_id, label, True, 0.0, float(threshold), note,
                    "unverified", 0.0, blocking,
                    used_course_codes=[result.code for result in matched], assumptions=[note], source=source,
                )
            current = float(max(known_marks))
            return RuleEvaluation(
                rule_id, label, current >= threshold, current, float(threshold),
                f"Best recorded mark is {int(current)}%; at least {threshold}% is required.",
                status, 1.0, blocking,
                used_course_codes=[result.code for result in matched], source=source,
            )

        if rule_type in {"course", "choose_n"} and rule.get("children"):
            children = [self.evaluate(child) for child in rule.get("children", []) if isinstance(child, dict)]
            required = int(rule.get("required", 1 if rule_type == "course" else len(children)))
            complete_children = [child for child in children if child.complete]
            used = sorted({code for child in complete_children for code in child.used_course_codes})
            child_status = _combine_status([child.status for child in complete_children], status)
            return RuleEvaluation(
                rule_id, label, len(complete_children) >= required,
                float(len(complete_children)), float(required),
        ...
        if rule_type in {"course", "choose_n", "all_courses"}:
            ...
            if rule_type == "course" and required == 1:
                detail = (
                    f"Completed via {completed[0]}." if completed else
                    "Complete one of: " + ", ".join(codes) + "."
                )
            else:
                detail = f"{len(completed)} of {required} required course choices completed."
                if len(completed) < required and missing:
                    detail += " Remaining options include: " + ", ".join(missing[:12]) + ("…" if len(missing) > 12 else "")
            return RuleEvaluation(
                rule_id, label, len(completed) >= required,
                float(len(completed)), float(required), detail,
                status, 1.0 if status == "verified" else 0.7, blocking,
        ...
        if rule_type == "approved_credit_pool":
            ...
            if allocation is None:
                ...
                return RuleEvaluation(
                    rule_id, label, optional_only, 0.0, maximum if optional_only else required, detail,
                    "unverified", 0.0, blocking, assumptions=[detail], source=source,
                )
            current = float(allocation.recognised_credits + allocation.provisional_credits)
            ...
            if provisional_codes:
                detail += " Transcript-only course(s) provisionally allocated: " + ", ".join(provisional_codes) + "."
                assumptions.append(allocation.note)
                confidence = 0.35
            else:
                detail += " Faculty approval and live registration eligibility still require confirmation."
                assumptions.append(allocation.note)
            complete = current >= required if required > 0 else (current <= maximum if maximum > 0 else True)
            display_required = required if required > 0 else maximum
            return RuleEvaluation(
                rule_id, label, complete, current, display_required, detail,
                evaluation_status, confidence, blocking,
        ...
        if rule_type == "credit_pool":
            ...
            required = float(rule.get("required", 0))
            matched = sorted(
                code for code, fact in self.fact_by_code.items()
                if (not explicit or code in explicit) and _fact_matches_filters(code, fact, filters)
            )
            current = float(sum(self.fact_by_code[code].nqf_credits for code in matched))
            ...
            if explicit and current < required:
                ...
                if remaining:
                    detail += " Remaining recorded options include: " + ", ".join(remaining[:12]) + ("…" if len(remaining) > 12 else "")
            return RuleEvaluation(
                rule_id, label, current >= required, current, required, detail,
                status, 1.0 if status == "verified" else 0.7, blocking,
        ...
        if rule_type == "maximum_credit_pool":
            ...
            current = float(sum(self.fact_by_code[code].nqf_credits for code in matched))
            return RuleEvaluation(
                rule_id, label, current <= maximum, current, maximum,
        ...
        if rule_type == "same_department_credit_pool":
            filters = rule.get("filters", {}) if isinstance(rule.get("filters", {}), dict) else {}
            transcript_filters = rule.get("transcript_filters", {}) if isinstance(rule.get("transcript_filters", {}), dict) else {}
            explicit = set(_normalise_codes(rule.get("course_codes", [])))
            required = float(rule.get("required", 0))
            grouped: dict[str, list[str]] = {}
            grouped_credits: dict[str, int] = {}
            for code, fact in self.fact_by_code.items():
                if (explicit and code not in explicit) or not _fact_matches_filters(code, fact, filters):
                    continue
                group = fact.department.strip() or re.match(r"^[A-Z]+", code).group(0)
                grouped.setdefault(group, []).append(code)
                grouped_credits[group] = grouped_credits.get(group, 0) + fact.nqf_credits
            ...
            if bool(rule.get("allow_unlisted_transcript_courses", False)):
                for result in self.student.credited_results():
                    if result.code in self.catalogue.courses or result.nqf_credits <= 0:
                        continue
                    pseudo = CourseFact(
                        code=result.code, name=result.name, nqf_credits=result.nqf_credits,
                        nqf_level=result.nqf_level, prerequisites=[], offered=[],
                        department=(re.match(r"^[A-Z]+", result.code).group(0) if re.match(r"^[A-Z]+", result.code) else ""),
                    )
                    if not _fact_matches_filters(result.code, pseudo, transcript_filters):
                        continue
                    group = re.match(r"^[A-Z]+", result.code).group(0) if re.match(r"^[A-Z]+", result.code) else ""
                    grouped.setdefault(group, []).append(result.code)
                    grouped_credits[group] = grouped_credits.get(group, 0) + result.nqf_credits
            ...
            if best_group in provisional_groups:
                ...
                confidence = 0.35
                assumptions.append("This discipline pool includes transcript-only courses whose programme recognition must be confirmed.")
            detail = f"{int(current)} of {int(required)} credits completed in one discipline"
            if best_group:
                detail += f" ({best_group})"
            detail += "."
            return RuleEvaluation(
                rule_id, label, current >= required, current, required, detail,
                evaluation_status, confidence, blocking, used,
                assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "no_failures":
            ...
            for result in self.student.results:
                ...
                if explicit and result.code not in explicit:
                    continue
                if fact is not None and filters and not _fact_matches_filters(result.code, fact, filters):
                    continue
                failures.append(result.code)
            complete = not failures
            note = "No failed course attempts are recorded." if complete else "Failed attempt(s) recorded for: " + ", ".join(sorted(set(failures))) + "."
            eval_status = status
            assumptions: list[str] = []
            if failures and bool(rule.get("condonable", False)):
                ...
                assumptions.append("Any condonation must be confirmed from an official Senate or faculty decision.")
            return RuleEvaluation(
                rule_id, label, complete, 0.0 if complete else float(len(set(failures))), 0.0,
                note, eval_status, 1.0 if eval_status == "verified" else 0.0, blocking,
                sorted(set(failures)), assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "passed_mark_equivalents":
            ...
            for code, result in self.result_by_code.items():
                ...
                if result.mark is None:
                    ...
                    equivalents += (fact.nqf_credits / unit) if unit else _course_weight(code)
                    used.append(code)
            eval_status = status if not unknown else _combine_status([status, "unverified"])
            assumptions = (["Numeric marks are missing for: " + ", ".join(sorted(unknown))] if unknown else [])
            return RuleEvaluation(
                rule_id, label, equivalents >= required, equivalents, required,
                f"{equivalents:g} of {required:g} full-course equivalents have marks of at least {threshold:g}%.",
                eval_status, 1.0 if not unknown else 0.0, blocking,
                sorted(used), assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "first_class_group":
            ...
            for code in codes:
                ...
                if result is None or fact is None:
                    continue
                attempts = [r for r in self.student.results if r.code == code and not r.is_pending()]
            ...
            candidates.sort(key=lambda row: (row[1].mark or -1, row[2].nqf_credits), reverse=True)
            selected = candidates[:required]
            marks = [float(row[1].mark) for row in selected if row[1].mark is not None]
            complete = len(selected) >= required and all(mark >= threshold for mark in marks)
            average_used = False
            if not complete and allow_average and len(selected) >= required:
                ...
                if len(credits) == 1:
                    ...
                    averaging_allowed = (required == 2 and credit in {24, 36}) or (required == 4 and credit == 12)
                    if averaging_allowed and min(marks, default=0) >= lower and sum(marks) / len(marks) >= threshold:
                        complete = True
                        average_used = True
            status_out = status
            assumptions: list[str] = []
            if unknown:
                status_out = _combine_status([status_out, "unverified"]); assumptions.append("Numeric marks are missing for: " + ", ".join(sorted(unknown)))
            if repeated_or_supp:
                assumptions.append("Repeated or supplementary passes do not count for Science distinction: " + ", ".join(sorted(repeated_or_supp)))
            detail = f"{len(selected)} of {required} first-attempt course results assessed; "
            if marks:
                detail += f"selected average {sum(marks)/len(marks):.2f}% and minimum {min(marks):.0f}%."
            else:
                detail += "no eligible numeric marks are available."
            if repeated_or_supp:
                detail += " Repeated or supplementary passes were excluded: " + ", ".join(sorted(repeated_or_supp)) + "."
            if average_used:
                detail += " The FB8.1(b) group-averaging concession was applied."
            return RuleEvaluation(
                rule_id, label, complete, float(len(selected)), float(required), detail,
                status_out, 1.0 if status_out == "verified" else 0.0, blocking,
                [row[0] for row in selected], assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "best_n_average":
            ...
            required = int(rule.get("required", 1))
            threshold = float(rule.get("minimum_average", rule.get("required_average", 0)))
            ...
            remaining = [row for row in candidates if row[0] not in {item[0] for item in selected}]
            ...
            average = sum(mark * weight for _, mark, weight in selected) / denominator if denominator else 0.0
            mandatory_met = mandatory <= {code for code, _, _ in selected}
            mark_count = sum(mark >= minimum_mark for _, mark, _ in selected) if minimum_mark else len(selected)
            complete = len(selected) >= required and mandatory_met and average >= threshold and mark_count >= minimum_mark_count
            eval_status = status
            assumptions: list[str] = []
            if missing_marks:
                eval_status = _combine_status([eval_status, "unverified"])
                assumptions.append("Numeric marks are missing for: " + ", ".join(sorted(missing_marks)))
            ...
            if mandatory and not mandatory_met:
                detail += " Mandatory course result(s) missing: " + ", ".join(sorted(mandatory - {code for code, _, _ in selected})) + "."
            if minimum_mark_count:
                detail += f" {mark_count} of {minimum_mark_count} required results meet {minimum_mark:g}%."
            return RuleEvaluation(
                rule_id, label, complete, average, threshold, detail,
                eval_status, 1.0 if eval_status == "verified" else 0.0, blocking,
                [code for code, _, _ in selected], assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "first_attempt_weighted_average":
            ...
            level_weights_raw = rule.get("level_weights", {}) if isinstance(rule.get("level_weights", {}), dict) else {}
            level_weights = {int(level): float(weight) for level, weight in level_weights_raw.items()}
            ...
            for code, result in first_by_code.items():
                ...
                normalised_grade = " ".join(str(result.grade or "").strip().upper().split())
            ...
            if denominator == 0:
                ...
                return RuleEvaluation(rule_id, label, False, 0.0, threshold, note, "unverified", 0.0, blocking, assumptions=[note], source=source)
            average = weighted_total / denominator
            eval_status = status
            assumptions: list[str] = []
            if unknown:
                eval_status = _combine_status([eval_status, "unverified"])
                assumptions.append("First-attempt numeric marks are missing for: " + ", ".join(sorted(unknown)))
            return RuleEvaluation(
                rule_id, label, average >= threshold, average, threshold,
        ...
        if rule_type == "weighted_average":
            ...
            missing_marks = [code for code in matched_codes if self.result_by_code[code].mark is None]
            ...
            if not marked:
                ...
                return RuleEvaluation(
                    rule_id, label, False, 0.0, threshold, note,
                    "unverified", 0.0, blocking,
                    used_course_codes=matched_codes + provisional_codes, assumptions=[note], source=source,
                )
            denominator = sum(weight for _, weight in marked)
            average = sum(result.mark * weight for result, weight in marked) / denominator
            evaluation_status = status
            assumptions: list[str] = []
            confidence = 1.0 if status == "verified" else 0.7
            if provisional_codes:
            ...
            if missing_marks:
                ...
                confidence = 0.0
                assumptions.append("Some passed courses have no numeric mark and were excluded: " + ", ".join(sorted(set(missing_marks))))
            return RuleEvaluation(
                rule_id, label, average >= threshold, average, threshold,
                f"Credit-weighted average is {average:.2f}%; at least {threshold:g}% is required.",
                evaluation_status, confidence, blocking,
                sorted(set(matched_codes + provisional_codes)), assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "passed_mark_count":
            ...
            for code, result in self.result_by_code.items():
                fact = self.fact_by_code[code]
                if code in excluded or (explicit and code not in explicit) or not _fact_matches_filters(code, fact, filters):
                    continue
                if result.mark is None:
                    unknown.append(code)
                elif result.mark >= threshold:
            ...
            assumptions = (["Numeric marks are missing for: " + ", ".join(sorted(unknown))] if unknown else [])
            return RuleEvaluation(
                rule_id, label, len(matched) >= required, float(len(matched)), float(required),
                f"{len(matched)} of {required} passed courses have marks of at least {threshold:g}%.",
                eval_status, 1.0 if not unknown else 0.0, blocking,
                sorted(matched), assumptions=assumptions, source=source,
            )
        ...
        if rule_type == "course_count":
            ...
            return RuleEvaluation(
                rule_id, label, len(matched) >= required,
                float(len(matched)), float(required),
        ...
        if rule_type in {"all_of", "any_of"}:
            ...
            if rule_type == "all_of":
                ...
                required = 1
            used = sorted({code for child in children if child.complete for code in child.used_course_codes})
            ...
            if not complete:
                ...
                if incomplete:
                    detail += " Outstanding: " + "; ".join(incomplete[:8]) + ("…" if len(incomplete) > 8 else "")
            return RuleEvaluation(
                rule_id, label, complete, float(current), float(required), detail,
                _combine_status(statuses), min([c.confidence for c in children] or [1.0]),
                blocking, used, source=source,
            )

        note = f"Unsupported curriculum rule type {rule_type!r}; manual verification is required."
        return RuleEvaluation(
            rule_id, label, True, 0.0, 1.0, note,
            "unverified", 0.0, blocking, assumptions=[note], source=source,
        )
