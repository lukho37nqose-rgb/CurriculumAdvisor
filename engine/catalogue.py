...
def load_catalogue(
    ...
    for raw in raw_courses:
        ...
        if not code:
            ...
            continue
        if code in courses:
            issues.append(f"Duplicate course definition for {code}; the later entry was used.")
        try:
        ...
        if credit_bearing and credits <= 0:
            issues.append(f"{code} has invalid or missing credit value: {raw.get('credits')!r}.")
        if not 1 <= level <= 10:
            issues.append(f"{code} has an invalid or unknown NQF level: {raw.get('nqf_level')!r}.")

        raw_prereqs = raw.get("prerequisites", [])
        prereq_valid = isinstance(raw_prereqs, list) and all(isinstance(p, str) for p in raw_prereqs)
        prereqs = [p.strip().upper() for p in raw_prereqs if isinstance(p, str) and p.strip()] if prereq_valid else []
        prereq_verified = bool(raw.get("prerequisites_verified", prereq_valid and (bool(prereqs) or level == 5)))
        raw_offered = raw.get("offered", [])
        offering_valid = isinstance(raw_offered, list) and all(isinstance(o, str) for o in raw_offered)
        offered = [o.strip() for o in raw_offered if o.strip()] if offering_valid else []
        co_reqs = [str(c).strip().upper() for c in raw.get("co_requisites", []) if str(c).strip()]
        ...
        courses[code] = CourseFact(
            ...
            counts_towards_general_degree=bool(raw.get("counts_towards_general_degree", True)),
            ...
            general_elective=bool(raw.get("general_elective", raw.get("counts_towards_general_degree", True))),
            counts_towards_course_equivalents=bool(raw.get("counts_towards_course_equivalents", True)),
    ...
    for key, raw in raw_reqs.get("majors", {}).items():
        ...
        for group in raw.get("choice_groups", []):
            courses_in_group = [str(c).strip().upper() for c in group.get("courses", []) if str(c).strip()]
            ...
            if required <= 0 or required > len(courses_in_group):
                issues.append(f"Major '{raw.get('name', key)}' has an invalid choice group requirement.")
        major = MajorDefinition(
            key=key,
            name=str(raw.get("name", key)),
            qualification=str(raw.get("qualification", "HUMANITIES")),
            required_courses=[str(c).strip().upper() for c in raw.get("required_courses", []) if str(c).strip()],
            ...
            curriculum_rules=[r for r in raw.get("curriculum_rules", []) if isinstance(r, dict)],
            stage_rules={
        ...
        majors[key] = major
        if not major.required_courses and not major.choice_groups and not major.curriculum_rules:
            issues.append(f"Major '{major.name}' has no representable requirements.")
        referenced_codes = major.required_courses + [c for g in major.choice_groups for c in g.courses]
        from .curriculum import collect_curriculum_course_codes
        referenced_codes += sorted(collect_curriculum_course_codes(major.curriculum_rules))
        ...
        for code in referenced_codes:
            if code not in courses:
                issues.append(f"{code} is referenced by major '{major.name}' but has no course definition.")
    ...
    for key, raw in raw_reqs.get("programmes", {}).items():
        ...
        for pathway_key, pathway_raw in raw.get("pathways", {}).items():
            ...
            pathways[pathway_key] = PathwayDefinition(
                ...
                curriculum_rules=[r for r in pathway_raw.get("curriculum_rules", []) if isinstance(r, dict)],
                required_courses=[str(c).strip().upper() for c in pathway_raw.get("required_courses", []) if str(c).strip()],
                support_course_codes=[str(c).strip().upper() for c in pathway_raw.get("support_course_codes", []) if str(c).strip()],
                verification_status=str(pathway_raw.get("verification_status", "verified")),
                availability=str(pathway_raw.get("availability", "open")),
                availability_note=str(pathway_raw.get("availability_note", "")),
                progression_rules=[r for r in pathway_raw.get("progression_rules", []) if isinstance(r, dict)],
                award_rules=[r for r in pathway_raw.get("award_rules", []) if isinstance(r, dict)],
                source=pathway_raw.get("source", {}) if isinstance(pathway_raw.get("source", {}), dict) else {},
            )
        ...
        for level, credits in raw.get("level_credit_requirements", {}).items():
            try:
                ...
                issues.append(f"Programme '{key}' has an invalid NQF-level credit requirement: {level!r} -> {credits!r}.")

        programmes[key] = ProgrammeRules(
            key=key,
            name=str(raw.get("name", key)),
            total_nqf_credits=int(raw.get("minimum_nqf_credits", raw.get("total_nqf_credits", 360))),
            level_7_nqf_credits=int(raw.get("minimum_nqf_level_7_credits", raw.get("level_7_nqf_credits", 120))),
            semester_course_equivalents=int(raw.get("minimum_semester_courses", raw.get("semester_course_equivalents", 20))),
            senior_course_equivalents=int(raw.get("minimum_senior_semester_courses", raw.get("senior_course_equivalents", 10))),
            humanities_course_equivalents=int(raw.get("minimum_humanities_semester_courses", raw.get("humanities_course_equivalents", 0))),
            required_majors=int(raw.get("minimum_majors", raw.get("required_majors", 0))),
            required_humanities_majors=int(raw.get("minimum_humanities_majors", raw.get("required_humanities_majors", 0))),
            max_courses_per_semester=float(raw["max_courses_per_semester"]) if raw.get("max_courses_per_semester") is not None else None,
            required_courses=[str(c).strip().upper() for c in raw.get("required_courses", []) if str(c).strip()],
            minimum_duration_years=int(raw.get("minimum_duration_years", 3)),
            maximum_registration_years=int(raw["maximum_registration_years"]) if raw.get("maximum_registration_years") else None,
            qualification_codes=[str(c) for c in raw.get("qualification_codes", [])],
            major_keys=[str(m) for m in raw.get("major_keys", [])],
            elective_course_codes=[str(c).strip().upper() for c in raw.get("elective_course_codes", []) if str(c).strip()],
            ...
            introductory_course_options=[str(c).strip().upper() for c in raw.get("introductory_course_options", [])],
            introductory_courses_required=int(raw.get("introductory_courses_required", 0)),
            augmenting_courses_required=int(raw.get("augmenting_courses_required", 0)),
            support_course_codes=[str(c).strip().upper() for c in raw.get("support_course_codes", []) if str(c).strip()],
            ...
            progression_rules=[r for r in raw.get("progression_rules", []) if isinstance(r, dict)],
            award_rules=[r for r in raw.get("award_rules", []) if isinstance(r, dict)],
            prerequisite_overrides={
                str(code).strip().upper(): [str(item).strip().upper() for item in values if str(item).strip()]
    ...
    forbidden = [tuple(pair) for pair in raw_reqs.get("forbidden_major_combinations", []) if isinstance(pair, list) and len(pair) == 2]
    return Catalogue(
        courses=courses,
        majors=majors,
